import {SkPoint, Skia, vec} from '@shopify/react-native-skia';
import {useCallback, useRef, useState} from 'react';
import {useSharedValue, withSpring} from 'react-native-reanimated';
import {IPaintStyle, Tools} from '@components/myClass/shared.types';
...
const useCanvas = ({
  ...
  const createShape = (x: number, y: number) => {
    switch (tool) {
      ...
      case Tools.circle:
        return {
          tool,
          coordinates: {cx: x, cy: y, r: sharedValueX},
          ...paintStyle,
        };
      case Tools.line:
  ...
  const onDrawingActive = useCallback(
    (touchInfo: any) => {
      ...
      if (tool === Tools.circle) {
        ...
        currentShape.current.coordinates.width.value = withSpring(
          x - shape.coordinates.x,
        );
        currentShape.current.coordinates.height.value = withSpring(
        ...
        currentShape.current.coordinates.path.quadTo(
          lastPoint.x,
          lastPoint.y,
          xMid,
          yMid,
        ...
        currentShape.current.coordinates.p2.value = {
          x: withSpring(x),
          y: withSpring(y),
        };
  ...
  const onDrawingEnd = useCallback(() => {
    setShapes(shapes => {
      ...
      if (tool === Tools.circle) {
        currentShape.current.coordinates.r =
          currentShape.current.coordinates.r.value;
      } else if (tool === Tools.square) {
        const shape = currentShape.current;
        currentShape.current.coordinates.width = shape.coordinates.width.value;
        currentShape.current.coordinates.height =
          shape.coordinates.height.value;
      } else if (tool === Tools.line) {
  ...
  const touchHandler = PanResponder.create({
    onStartShouldSetPanResponder: () => true,
    onPanResponderGrant: (e: GestureResponderEvent) => {
      onDrawingStart({x: e.nativeEvent.locationX, y: e.nativeEvent.locationY});
    },
    onPanResponderMove: (e: GestureResponderEvent) => {
      onDrawingActive({x: e.nativeEvent.locationX, y: e.nativeEvent.locationY});
    },
