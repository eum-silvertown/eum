...
function ClassHomeworkStudentSubmitListScreen(): React.JSX.Element {
  const route = useRoute();
  const navigation = useNavigation();
  const { homeworkId } = route.params as { homeworkId: number };
  const lectureId = useLessonStore((state) => state.lectureId);
  const studentsCnt = useLectureStore((state) => state.lectureStudents);

  const { data: submissions, isLoading, isError } = useQuery<HomeworkStudentSubmission[]>({
    queryKey: ['homeworkSubmissions', lectureId, homeworkId],
    queryFn: () => getHomeworkSubmissions(lectureId!, homeworkId),
  });
  ...
  const renderSubmissionItem = ({ item }: { item: HomeworkStudentSubmission }) => (
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={submissions}
        keyExtractor={(item) => item.homeworkSubmissionId.toString()}
...
export default ClassHomeworkStudentSubmitListScreen;
