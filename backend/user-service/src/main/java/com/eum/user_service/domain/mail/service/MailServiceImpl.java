...
public class MailServiceImpl implements MailService{
    ...
    @Override
    @Transactional
    public void findPasswordWithAuthentication(EmailAuthCheckRequest emailAuthCheckRequest) {
        validateAuthenticationCode(emailAuthCheckRequest);
        Member member = userRepository.findByEmail(emailAuthCheckRequest.email())
                .orElseThrow(() -> new EumException(ErrorCode.USER_NOT_FOUND));
        emailValidationCodeRepository.deleteById(emailAuthCheckRequest.email());
        String temporaryPassword = sendTemporaryPasswordMessage(emailAuthCheckRequest.email());
        //임시비밀번호로 업데이트
    ...
    private String sendTemporaryPasswordMessage(String email) {
        String code = createCode();
        try {
            MimeMessage message = mailSender.createMimeMessage();
            message.setRecipients(MimeMessage.RecipientType.TO, email);
            message.setSubject("Eum 임시비밀번호 안내입니다.");
            String body = "";
